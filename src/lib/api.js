...
export const api = {
    ...
    fetchMetadata: async () => {
        ...
        metadataPromise = (async () => {
            ...
            const cities = Array.from(new Set(guides.map((g) => g.location.split(',')[0].trim()))).sort();
     ...
     */
    getReviewsForGuide: async (guideId) => {
     ...
     */
    getBookingRequests: async (guideId) => {
     ...
     */
    updateAvailability: async (guideId, date, status) => {
        const res = await fetch(`${API_URL}/api/availability/${guideId}`, {
            method: 'POST',
            headers: api.getHeaders(),
    ...
    addFavorite: async (userId, guideId) => {
    ...
    removeFavorite: async (userId, guideId) => {
    ...
    getSavedItineraries: async (userId) => {

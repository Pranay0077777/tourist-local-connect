...
// --- FAVORITES ---
export const toggleFavorite = (userId, guideId) => {
    const users = getUsers();
...
export const getMessages = (userId, contactId) => {
    ...
    return messages.filter((m) => (m.senderId === userId && m.receiverId === contactId) ||
        (m.senderId === contactId && m.receiverId === userId)).sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
};
...
export const markMessagesAsRead = (userId, contactId) => {
    const messages = JSON.parse(localStorage.getItem('tlc_messages') || '[]');
...
export const getBookingsForGuide = (guideId) => {
    const all = getAllBookings();

...
io.on('connection', (socket) => {
    ...
    socket.on('send_message', async (data) => {
        try {
            ...
            if (receiverIsGuide) {
                const isDemoGuide = receiverId.startsWith('guide_');
                let shouldRespondWithAI = isDemoGuide;
                
                if (!isDemoGuide) {
                    const msgCount = await db.queryOne(
                        'SELECT COUNT(*) as count FROM messages WHERE sender_id = ? AND receiver_id = ?', 
                        [senderId, receiverId]
                    );
                    if (msgCount && parseInt(msgCount.count) <= 1) {
